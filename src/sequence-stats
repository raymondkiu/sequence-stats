...
usage () {
  ...
  echo " -f Filter FASTA sequences by length. Usage: ./sequence-stats -f FASTA 500 > NEWFILENAME"
  echo " -h Print usage and exit"
  echo " -v Print version and exit"
  echo ""
  echo "Version 1.1 (2023)"
  echo "Author: Raymond Kiu Raymond.Kiu@quadram.ac.uk"
  echo "";
}

version (){
echo "sequence-stats 1.1"
}
...
awk -v n=$name '/^>/{ if(l>n) print b; b=$0;l=0;next } {l+=length;b=b ORS $0}END{if(l>n) print b }' $FILE
...
# Call options
while getopts ':aqtcdnbresfhv' opt;do
  case $opt in
    a) fasta; exit;;
    q) fastq; exit;;
    t) fastq2fasta; exit;;
